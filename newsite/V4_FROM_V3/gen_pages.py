...
SUBSET = [ "home",\
	...
	"map", \
	"partners", \
	"etcetera", "mocap", "axe", "citypoly", \
	"animation", "animation_gallery", \
	"motiondesign","motiondesign_gallery", \
	"previs", "stbd_artists", "anim_cinem", "char_dev",  "storyboard", \
	"appdesign", "website", "touchscreen", "interactive", \
	"photos", \
	"contacts" ]

#SUBSET = [ "whoweare", "contacts", "clients", "sneakpeek" ]
...
#SUBSET = [ "sneakpeek", "contacts" ]
#SUBSET = [ "appdesign", "website", "touchscreen", "interactive" ]
#SUBSET = [ "etcetera", "motion" ]
#SUBSET = [ "photos" ]
#SUBSET = [ "home"]
#SUBSET = [ "etcetera" ]
#SUBSET = [ "whoweare", "partners", "community", "animation", "animation_gallery", "interactive", "appdesign","website","touchscreen" ]
#SUBSET = [ "whoweare"]
#SUBSET = [ "partners"]
#SUBSET = [ "whoweare", "partners", "community" ]
#SUBSET = [ "sneakpeek" ]
#SUBSET = [ "appdesign", "website", "touchscreen", "interactive", "etcetera", "motion" ]
#SUBSET = [ "etcetera", "mocap", "axe", "citypoly", "home" ]
#SUBSET = [ "contacts" ]
#SUBSET = [ "home" ]
		...
		if asset_name.startswith("mov"):
			...
			clickpanel_cb = None
					...
					ahref = link	
                                        ...
                                        print "ERROR: gen_pages: Unknown link type", asset_name, item
                                        sys.exit(1)

			style, content, top_script, scriptlet_dct = gen_images.expand_item( accum_ids, item, images_dct, script, None, \
				ahref, exturl, clickpanel_cb )
 
		elif asset_name.startswith("embed"):

			print "MAIN - CALLING GEN EMBED", asset_name, item
			style, content, top_script, scriptlet_dct = gen_embeds.expand_item( accum_ids, item, embeds_dct, images_dct, None, None )
			...
			style, content, top_script, scriptlet_dct  = gen_menus.expand_item( accum_ids, item, images_dct, menus_dct, \
				slide_shows_dct, movies_dct, movie_panels_dct, image_sets_dct, click_panels_dct, cpo_dct, embeds_dct )
			...
			style, content, top_script, scriptlet_dct = \
				...
				click_panels_dct, slide_shows_dct, cpo_dct, embeds_dct )
...
if __name__ == "__main__":
	...
	for page_name in pagekeys:
		...
		if page_def[0]['asset_name'] == "mpg_animation_gallery":
			gen_multipage_gallery.gen_pages( page_def[0], template_style, template_content, movie_panels_dct, movies_dct, images_dct, embeds_dct )
			...
			for item_def in page_def:
				if ( item_def['asset_name'].startswith("mss_") ):
					gen_multipage_slideshow.gen_pages( item_def, template_style, template_content, movie_panels_dct, movies_dct, \
						images_dct, image_sets_dct, embeds_dct )
				else:
			...
			# get lightbox stuff...
			head_tail = ""
			if ( page_name == "sneakpeek" ):
				lbh = open("lightbox/head.txt")
				lb_head = lbh.read()
				lbh.close()
				head_tail += lb_head
			...
			# write the file...
			common.gen_page( "%s.html" % page_name, style, head_tail, content, head_script, load_script )
